...
export default function PlayerProfilePage({ params }: PlayerProfilePageProps) {
  ...
  const fetchPlayerData = async () => {
    ...
    // Check if player is shortlisted (for agents)
  ...
  if (!player) {
    return (
      <div className="text-center py-12">
        ...
        <Link href={fromAdmin ? "/dashboard/admin" : "/dashboard/players"} className="text-red-600 hover:underline mt-4 inline-block">
          Back
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-6xl mx-auto">
      {/* Back Button - Returns to correct page based on user role */}
      <Link 
        href={fromAdmin ? "/dashboard/admin" : (userRole === 'agent' ? "/dashboard/players" : "/dashboard/players")} 
        className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-6"
      >
        <ArrowLeft className="w-4 h-4 mr-1" />
        Back to {fromAdmin ? 'Admin Panel' : 'Players'}
      </Link>
